        ...
        <div class="add-filter-menu" :class="{ 'add-filter-menu-extended': showErrorMessage }">
            <div class="error-notification" v-show="showErrorMessage">
                <div class="error border-2 border-red-500 bg-red-200 rounded text-center py-2 mb-2">
                    <p class="text-red-500 font-bold">Niet alle velden zijn ingevuld. Vul een titel in en verwijder lege
                        filters!</p>
                </div>
            </div>
            <div class="head">
            ...
            <div class="body">
                ...
                <div class="filter-options">
                    <div class="title">
                        <h2>Filter opties:</h2>
                        <div class="add-filter cursor-pointer" @click="_handleAddFilterClick">
                            <i class="fas fa-plus"></i>
                        </div>
                    </div>
                    <div class="list-filter-options">
                        <dm-added-filter v-for="filter in filterOptions" :key="filter.id" :filter="filter"
                            @_handleRemoveFilter="_handleRemoveFilter"
                            @_updateFilterValue="_updateFilterValue"></dm-added-filter>
                    </div>
                    <div class="actions">
                        <div @click="_handleRejectNewFilterGroup"
                            class="reject bg-gradient-to-b w-1/8 cursor-pointer from-red-500 flex items-start justify-between to-red-200 border-b-4 border-red-500 rounded-lg shadow-xl p-3">
                            <div
                                class="inner flex rounded-lg shadow-xl py-2 px-5 border-2 border-red-500 bg-red-200 text-red-500 font-bold">
                                Annuleren</div>
                        </div>
                        <div @click="_handleAcceptNewFilterGroup"
                            class="accept bg-gradient-to-b cursor-pointer w-1/8 from-green-500 flex items-start justify-between to-green-200 border-b-4 border-green-500 rounded-lg shadow-xl p-3">
                            <div
                                class="inner flex rounded-lg shadow-xl py-2 px-5 border-2 border-green-500 bg-green-200 text-green-500 font-bold">
                                Opslaan</div>
                        </div>
                    </div>
                </div>
