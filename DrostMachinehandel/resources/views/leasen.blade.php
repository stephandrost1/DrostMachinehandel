...
<div class="px-5 md:px-20 py-7">
  ...
  <div class="mb-5">
    <h1 class="font-bold text-3xl">{{ __('content/lease.title') }}</h1>
  </div>
  ...
  <div class="flex flex-wrap gap-10">
    <div style="flex: 1 0 21%;" class="h-fit grow-0 border-primary border-[3px] rounded-xl p-5 min-w-full sm:min-w-[440px] order-2 lg:order-1">
        <div>
            <h1 class="text-2xl font-bold mb-2">{{ __('content/lease.title') }}</h1>
            <p>{{ __('content/lease.subtitle 1 text') }}</p>
        </div>
        ...
        <form class="mt-4" action="">
            ...
            <div class="flex flex-col gap-5">
                <div>
                    <label for="aanschafwaarde" class="block mb-2 text-white font-[800]">{{ __('content/lease.input 1') }}</label>
                ...
                <div>
                    <label for="aanschafwaarde" class="block mb-2 text-white font-[800]">{{ __('content/lease.input 2.1') }} <span class="font-normal text-secondary">{{ __('content/lease.input 2.2') }}</span></label>
                    <div class="flex w-5/12 sm:w-3/12">
                ...
                <div>
                    <label for="aanschafwaarde" class="block mb-2 text-white font-[800]">{{ __('content/lease.input 3.1') }} <span class="font-normal text-secondary">{{ __('content/lease.input 3.2') }}</span></label>
                    <div class="flex w-5/12 sm:w-3/12">
                        <span class="inline-flex items-center px-3 text-white font-bold bg-transparent rounded-l-md border-2 border-r-0 border-secondary">
                            <i class="fas fa-euro-sign fa-xs"></i>
                        </span>
            ...
            <button type="submit" class="mt-5 w-32 border-[3px] border-primary bg-primary text-white font-bold px-24 py-1 rounded-lg flex gap-5 items-center justify-center">{{ __('content/lease.calculate button text') }} <i class="fas fa-calculator"></i></button>
    ...
    <div style="flex: 1 0 21%;" class="grow-0 border-primary border-[3px] rounded-xl p-5 h-fit min-w-full sm:min-w-[440px] order-1 lg:order-2">
        ...
        <div>
            <h1 class="text-2xl font-bold mb-3">{{ __('content/lease.subtitle 2') }}</h1>
            <p>{{ __('content/lease.subtitle 2 text') }}</p>
        </div>
